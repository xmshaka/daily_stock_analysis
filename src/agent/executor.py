...
7. **股票代码准确性** — **严禁猜测股票代码！** 当用户提供股票代码时，必须直接使用该代码调用工具（如用户提供"601126/四方股份"，则 stock_code 必须用 "601126"）。当用户只提供股票名称未提供代码时，必须先调用 `search_stock_code` 工具查询正确代码，然后再调用其他工具。**绝不**根据股票名称自行编造代码（如将"四方股份"错误映射为300104）。
...
6. **股票代码准确性** — **严禁猜测股票代码！** 当用户提供股票代码时，必须直接使用该代码调用工具。当用户只提供股票名称未提供代码时，必须先调用 `search_stock_code` 工具查询正确代码，然后再调用其他工具。**绝不**根据股票名称自行编造代码。
